...
# Stage 1-4 at 90%+ use their WebM bonus scenes in the clear modal.
rep(Path('game.js'),
...
"""  const continueBtn=!perfect?`<button id="keepBtn" class="btn secondary">KEEP PLAYING · ${target}</button>`:'';
  const artSrc=stageSrc(state.stage);
  const bonusSceneSrc=state.stage===1?'assets/bonus/stage1_maid_bonus.webm':state.stage===2?'assets/bonus/stage2_racinggirl_bonus.webm':state.stage===3?'assets/bonus/stage3_officelady_bonus.webm':state.stage===4?'assets/bonus/stage04_miko_bonus.webm':'';
...
/* Stage 1-4 90%+ WebM bonus scenes */
...
print('stage1-4 bonus scene patch applied')
