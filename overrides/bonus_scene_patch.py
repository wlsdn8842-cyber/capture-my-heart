...
# Stage 1 at 90%+ uses the WebM bonus scene in the clear modal.
rep(Path('game.js'),
...
"""  const continueBtn=!perfect?`<button id="keepBtn" class="btn secondary">KEEP PLAYING · ${target}</button>`:'';
  const artSrc=stageSrc(state.stage);
  const hasBonusScene=state.stage===1&&level>=90;
  const mediaHtml=hasBonusScene
    ? `<video id="clearBonusVideo" class="clear-photo clear-bonus-video" src="assets/bonus/stage1_maid_bonus.webm" autoplay muted loop playsinline preload="auto" poster="${artSrc}"></video>
...
print('stage1 bonus scene patch applied')
